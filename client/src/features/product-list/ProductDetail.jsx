...
import { fetchProudctByIdAsync, selectProductById } from './productSlice'
...
export default function ProductDetail({handleOrderPopup}) {
  const [selectedColor, setSelectedColor] = useState(colors[0])
  const [selectedSize, setSelectedSize] = useState(sizes[2])
  const product = useSelector(selectProductById);
  const user = useSelector(selectLoggedInUser);
  const cartItemsByUser = useSelector(selectItems)
  ...
  const handleCart = (e) => {
      e.preventDefault();
      if (cartItemsByUser.findIndex(item=>item.product.id === product.id)<0) {
  ...
  useEffect(() => {
    dispatch(fetchProudctByIdAsync(params.id))
  }, [dispatch,params.id]);
  return (
    <div className=" bg-[#fff]">
      {product && <div className="pt-6">
        ...
        <nav aria-label="Breadcrumb ">
          <ol role="list" className="mx-auto flex max-w-2xl items-center space-x-2 px-4 sm:px-6 md:max-w-7xl md:px-8">
            {product.breadcrumbs && product.breadcrumbs.map((breadcrumb) => (
              <li key={breadcrumb.id}>
                <div className="flex items-center">
                  <a href={breadcrumb.href} className="mr-2 text-sm font-medium text-gray-900 capitalize">
                    {breadcrumb.title}
                  </a>
                  <svg
                    width={16}
            ...
            <li className="text-sm">
              <a href={product.href} aria-current="page" className="font-medium text-3xl text-gray-900 hover:text-gray-600 font-Raleway">
                {product.title}
              </a>
            </li>
          </ol>
        ...
        <div className="mx-auto mt-6 max-w-2xl sm:px-6 md:grid md:max-w-7xl md:grid-cols-3 md:gap-x-8 md:px-8">
          <div className="aspect-h-4 aspect-w-3 hidden overflow-hidden rounded-lg md:block">
            <img
          ...
          <div className="aspect-h-5 aspect-w-4 md:aspect-h-4 md:aspect-w-3 sm:overflow-hidden sm:rounded-lg">
            <img
        ...
        <div className="mx-auto max-w-2xl px-4 pb-16 pt-10 sm:px-6 md:grid md:max-w-7xl md:grid-cols-3 md:grid-rows-[auto,auto,1fr] md:gap-x-8 md:px-8 md:pb-24 md:pt-16">
          ...
          <div className="py-10 md:col-span-2 md:col-start-1 md:border-r md:border-gray-200 md:pb-16 md:pr-8 md:pt-6">
...
optio, eaque rerum!
            ...
            <div className="mt-10">
              ...
              <div className="mt-4 space-y-6">
                <p className="text-sm text-gray-600">Provident similique accusantium nemo autem. Veritatis
