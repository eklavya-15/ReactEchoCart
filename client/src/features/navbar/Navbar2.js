    ...
    <div className="w-full">
      <div className="flex flex-wrap items-center justify-between mx-auto p-4 bg-[#111827] text-white z-20 top-0 start-0 border-b border-gray-200">
        ...
        <div className="sm:flex sm:flex-row hidden text-xl items-center justify-center gap-2 md:order-2 rtl:space-x-reverse">
          <div className="ml-4 flex items-center md:ml-6">
            <Link to="/cart">
              <HiOutlineShoppingCart className="h-6 w-6 cursor-pointer text-green-600 mr-2" aria-hidden="true" />
            </Link>
            {items.length > 0 && (
                ...
                <Menu.Items className="absolute right-0 z-10 mt-2 w-48 origin-top-right rounded-md bg-white py-1 shadow-lg ring-1 ring-black ring-opacity-5 focus:outline-none">
                  {userNavigation.map((item) => (
                    <Menu.Item key={item.name}>
                      <Link to={item.link} className="block px-4 py-2 text-sm text-gray-700">
                        {item.name}
                      </Link>
                    </Menu.Item>
                  ))}
        ...
        <div className="-mr-2 flex sm:hidden">
          <Disclosure.Button className="relative inline-flex items-center justify-center rounded-md bg-gray-800 p-2 text-gray-400 hover:bg-gray-700 hover:text-white focus:outline-none focus:ring-2 focus:ring-white focus:ring-offset-2 focus:ring-offset-gray-800">
            {isOpen ? (
              <XMarkIcon className="block h-6 w-6" aria-hidden="true" onClick={handleClick} />
            ) : (
              <Bars3Icon className="block h-6 w-6" aria-hidden="true" onClick={handleClick} />
            )}
          </Disclosure.Button>
        </div>
        ...
        <div className={`items-center justify-between w-full md:flex md:w-auto md:order-1 hidden`} id="navbar-sticky">
          <ul className="text-white flex flex-col p-4 md:p-0 mt-4 font-medium border border-gray-100 rounded-lg bg-[#111827] md:space-x-8 rtl:space-x-reverse md:flex-row md:mt-0 md:border-0 md:bg-[#111827] ">
            <li className="flex flex-row items-center justify-center text-white md:hover:text-green-500">
              <IoManSharp />
              <Link to="/navbar/men" className="block py-2 px-3 text-white md:hover:text-green-500 bg-blue-700 rounded md:bg-transparent md:text-white md:p-0 md:dark:text-blue-500 ml-2" aria-current="page">
                MEN
              </Link>
            </li>
            <li className="flex flex-row items-center justify-center md:hover:text-green-500">
              <IoWomanSharp />{" "}
              <Link to="/navbar/women" className="block py-2 px-3 text-white rounded hover:bg-gray-100 md:hover:text-green-500 md:hover:bg-transparent md:p-0 md:dark:hover:text-blue-500 dark:text-white dark:hover:bg-gray-700 dark:hover:text-white md:dark:hover:bg-transparent dark:border-gray-700 ml-2">
                WOMEN
              </Link>
            </li>
            <li className="flex flex-row items-center justify-center md:hover:text-green-500">
              <GiWatch />
              <Link to="/navbar/watches" className="block py-2 px-3 text-white rounded hover:bg-gray-100 md:hover:bg-transparent md:p-0 md:hover:text-green-500 md:dark:hover:text-blue-500 dark:text-white dark:hover:bg-gray-700 dark:hover:text-white md:dark:hover:bg-transparent dark:border-gray-700 ml-1">
                WATCHES
              </Link>
            </li>
            <li className="flex flex-row items-center justify-center md:hover:text-green-500">
              <GiFlowerPot />
              <Link to="/navbar/home-decor" className="block py-2 px-3 text-white rounded hover:bg-gray-100 md:hover:bg-transparent md:hover:text-green-500 md:p-0 md:dark:hover:text-blue-500 dark:text-white dark:hover:bg-gray-700 dark:hover:text-white md:dark:hover:bg-transparent dark:border-gray-700 ml-1">
                HOME DECOR
              </Link>
            </li>
          </ul>
      ...
      <Disclosure.Panel className="md:hidden">
        <div className="space-y-1 px-2 pb-3 pt-2 sm:px-3">
          {navigation.map((item) => (
            <Disclosure.Button key={item.name} className={classNames(item.current ? "bg-gray-900 text-white" : "text-gray-300 hover:bg-gray-700 hover:text-white", "block rounded-md px-3 py-2 text-base font-medium")}>
